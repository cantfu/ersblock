...
/*改写
 * 添加：构造方法中添加ShowSelfPanel面板
 * 功能：一边下落，一边演示提取到的Data对象信息。
 * 2017年10月19日09:29:02
 * */
...
public class ErsBlock extends Thread {
	 ...
	 */
	public ErsBlock(GameCanvas canvas, int style, int y, int x, int level,ObjectOutputStream out) {
		this.canvas = canvas;
		this.style = style;
		this.y = y;
		this.x = x;
		...
		recData = new int[canvas.rows][canvas.cols];
		int key = 0x8000;
	...
	public void run() {
		...
		while (moving) {
			...
			//
			recData = canvas.getCanvasData();
			data = new Data(recData, 0, 0);
			try {
				out.writeObject(data);//输出给Server服务端
			} catch (IOException e) {
